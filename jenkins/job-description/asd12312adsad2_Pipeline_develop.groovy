...
def job_config = [
        job: [
                name: "asd12312adsad2_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asd12312adsad2", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asd12312adsad2_Pipeline_develop");
